...
HDA="$(cat $HOME/.hddtemp.txt | grep "sda" | tr -d '/' | sed 's/sda|WDC WD15EARS-00MVWB0//g' | cut -f2 -d "|")"
HDB="$(cat $HOME/.hddtemp.txt | grep "sdb" | tr -d '/' | sed 's/sdb|Hitachi HDP725032GLA360//g' | cut -f2 -d "|")"
HDC="$(cat $HOME/.hddtemp.txt | grep "sdc" | tr -d '/' | sed 's/sdc|WDC WD10EACS-00D6B1//g' | cut -f2 -d "|")"
HDD="$(cat $HOME/.hddtemp.txt | grep "sdd" | tr -d '/' | sed 's/sdd|WDC WD10EACS-00D6B1//g' | cut -f2 -d "|")"
