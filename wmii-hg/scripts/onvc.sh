...
# Variables
NPV=$(ps x | grep vlc | head -1 | awk '{print $6}')
#
if [ "$NPV" == "" ];then echo -e "${W}On VLC${K}: ${BW} Empty.. ${R}[${BW}Nothing Playing${R}]"
else
if [ "$NPV" == "/dev/video0" ];then echo -e "${W}On VLC${K}: ${BW}Watching TV aka.. ${R}[${BW}IdiotBox${R}]"
else
